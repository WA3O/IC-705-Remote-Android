# IC-705 Remote Android - v28.41 feature guide
...
Receive audio uses 48 kHz, 16-bit mono PCM. Network status indicates GOOD, FAIR, POOR, CONNECTING or DISCONNECTED based on the streams. The app requests missing transport packets and can restart audio separately from control/spectrum, escalating to full session recovery when both streams stall.
...
Enter a frequency and press SET; readback verifies tuning. Step controls move by 500 Hz or 1 kHz. Tap the spectrum/waterfall to tune, or swipe horizontally once per gesture: right adds 1 kHz and left subtracts 1 kHz. Vertical, cancelled and multi-touch gestures do not tune.
